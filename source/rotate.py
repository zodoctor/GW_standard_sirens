...
"""
#
# if you want to move a map through and angle to center on the origin:
#
def rotateSkyToOrigin (ra,dec, alpha, beta) :
    alpha = -alpha
    beta = -beta
    lon = ra; lat = dec
    ...
    ix = lon > 180; lon[ix] = lon[ix]-360 ;# center on 0,0
    return lon,lat
# if you want to move the origin to center on a map position
def rotateSkyAwayFromOrigin (ra,dec, alpha, beta) :
    lon = ra; lat = dec
    x,y,z = sphericalToCartesian(lon,lat)
    x,y,z = rotateAboutYaxis(x,y,z, beta)
    x,y,z = rotateAboutZaxis(x,y,z, alpha)
